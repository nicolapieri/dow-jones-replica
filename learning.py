...
ml_model.fit(train_TE[['NNLS', 'PCRR', 'DTW', 'NNMF']], train_TE['Replica'])
...
t1 = time.time()
dl_model.fit(train_TE[['NNLS', 'PCRR', 'DTW', 'NNMF']], train_TE['Replica'], epochs=epochs, batch_size=10)
t2 = time.time()
print("-" * 100)
...
replica_preds['ML_preds'] = pd.Series(ml_model.predict(test_TE[['NNLS', 'PCRR', 'DTW', 'NNMF']]), index=test_TE.index)
replica_preds['DL_preds'] = (dl_model.predict(test_TE[['NNLS', 'PCRR', 'DTW', 'NNMF']]) > 0.5).astype(bool)
